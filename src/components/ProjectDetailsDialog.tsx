...
const ProjectDetailsDialog: React.FC<ProjectDetailsDialogProps> = ({
  ...
  // Component for displaying member avatar
  const MemberAvatar = ({ member }: { member: any }) => {
    const { avatarUrl } = useAvatar(member.user.id);
    return (
  ...
  // Component for displaying creator avatar
  const CreatorAvatar = () => {
    const { avatarUrl } = useAvatar(project.creatorId);
    return (
